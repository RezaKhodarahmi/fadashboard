...
import { updateCoupon } from 'src/store/apps/coupon'
...
export default function CrateUserForm(props) {
  ...
  //Get data from props and update inputs
  useEffect(() => {
    if (props.couponData) {
  ...
  const generateCouponCode = () => {
    ...
    // Set the generated code into the "Coupon Code" input
    // by setting the value of the input field
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} mb={3}>
          <Box display='flex' alignItems='center'>
            <TextField {...register('code')} label='Coupon Code' fullWidth />
            {errors.code && (
